...
const query = require('./query/dbbank');
const mutation = require('./mutation/dbbank');
...
module.exports = {
  Date: new GraphQLScalarType({
    ...
    serialize(value) { // value sent to the client
      return `${new Date(value).getFullYear()}-${getTwoDigit(new Date(value).getMonth() + 1)}-${getTwoDigit(new Date(value).getDate())}`;
    },
    parseLiteral(ast) {
