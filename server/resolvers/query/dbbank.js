...
const customerAttr = (filter, target) => {
  const targets = Object.keys(filter)
    .filter((key) => {
...
module.exports = {
  dbbankCount: async (obj, args, ctx) => {
    ...
    return await ctx.dbbankCustomers
      .find({
        $and: customerGlobal(filter).concat(customerAttr(filter)),
      })
      .countDocuments();
  ...
  dbbankTargetItems: async (obj, args, ctx) => {
    ...
    const group = history
      ? [{ $unwind: '$histories' }, { $match: keyword ? { [`histories.${target}`]: new RegExp(keyword, 'gi') } : {} }, { $group: { _id: `$histories.${target}` } }]
      : [{ $match: keyword ? { [target]: new RegExp(keyword, 'gi') } : {} }, { $group: { _id: `$${target}` } }];

    // console.log(JSON.stringify(filter))

    return await ctx.dbbankCustomers.aggregate(
      [{ $match: { $and: customerGlobal(filter).concat(customerAttr(filter, target)) } }].concat(group).concat([{ $sort: { _id: 1 } }, { $skip: skip }, { $limit: target === 'from' ? 150 : 40 }])
    );
  },

  dbbankSettings: async (obj, args, ctx) => {
    const { _id, target, name, value } = args;

    return await ctx.dbbankSettings
      .find({
        target,
      })
      .sort({
  ...
  dbbankNewCustomers: async (obj, args, ctx) => {
    return await ctx.dbbankNewCustomers.find().countDocuments();
  },
};
